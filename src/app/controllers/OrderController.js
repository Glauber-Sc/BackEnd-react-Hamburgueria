...
class OrderController {
    async store(request, response) {
        ...
        try {
            ...
            return response
                .status(500)
                .json({ error: 'Falha ao criar ordem' });
        }
    }
        ...
        try {
            ...
            const formattedOrders = orders.map((order) => {
                ...
                return {
                    ...
                    status: order.status,
                    createdAt: order.createdAt, // Incluída a data de criação do pedido
                };
            });
    ...
    async update(request, response) {
        ...
        try {
          ...
          order.status = status;
          //order.createdAt = addHours(new Date(), -3); // Ajuste manual da data
          await order.save();
